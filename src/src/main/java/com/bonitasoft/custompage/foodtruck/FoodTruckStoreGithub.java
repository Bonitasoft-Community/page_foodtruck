...
public class FoodTruckStoreGithub implements FoodTruckIntBonitaStore {
    ...
    private final static BEvent NoListingFound = new BEvent(FoodTruckStoreGithub.class.getName(), 2, Level.APPLICATIONERROR, "No Listing Found",
            "The Githbub repository is supposed to have a file 'listing.xml' which describe all objects available. THis file is not found.",
            "Check the github repository, you may be not access the correct one ? ");

    private final static BEvent BadListingFormat = new BEvent(FoodTruckStoreGithub.class.getName(), 3, Level.APPLICATIONERROR, "Bad Listing.xml format",
            "The format of the file Listing.xml is incorrect. This file describe all applications available in the repository",
            "The list of apps is not visible",
            "Check the exception, call the Github Administrator");
    private final static BEvent NoGithubInformation = new BEvent(FoodTruckStoreGithub.class.getName(), 4, Level.APPLICATIONERROR, "NoGithub information",
            "To connect to Gitghub, a login/password is mandatory",
            "The repository can't be accessed, the list of apps present itself is not visible",
            "Give a login / password");

    private final static BEvent noContribFile = new BEvent(FoodTruckStoreGithub.class.getName(), 5, Level.APPLICATIONERROR, "No contrib file",
            "The apps does not have any contrib file, and nothing can be download",
            "The apps can not be upload",
            "Contact the owner of the page to fix it");

    private final static BEvent noBinaryFile = new BEvent(FoodTruckStoreGithub.class.getName(), 6, Level.APPLICATIONERROR, "No binary file",
            "The binary file can't be uploaded from the store",
            "Apps can not be upload",
            "Contact the owner of the page to fix it");

    private final static BEvent errorDecodeLogo = new BEvent(FoodTruckStoreGithub.class.getName(), 7, Level.APPLICATIONERROR, "Error decode Logo",
            "The Logo can't be decode, or at not present",
            "No logo image",
            "Contact the owner of the page to fix it");

    private final static BEvent errorDecodePageProperties = new BEvent(
            FoodTruckStoreGithub.class.getName(),
            8,
            Level.APPLICATIONERROR,
            "Error decode Pageproperties",
            "The page.properties file can't be decode",
            "No description on the item in the repository, and then a local application and the repository one can be considered different when in fact this is the same",
            "Contact the owner of the page to fix it");
    ...
    public FoodTruckResult getListAvailableItems(final TypeApps typeApps, final LogBox logBox)
    {
        ...
        for (final Object oneRepositoryOb : resultListRepository.getJsonArray()) {
            ...
            // set the name. If a page.properties exist, then we will get the information inside
            ...
            // release is : : "https://api.github.com/repos/Bonitasoft-Community/page_awacs/releases{/id}",
         ...
         * appsItem.urlDownload = baseUrlDownload + eElement.getAttribute("bonitacontributfile");
         * appsItem.documentationFile = eElement.getAttribute("documentationfile");
         * try
         * {
         * appsItem.releaseDate = sdfParseRelease.parse(eElement.getAttribute("releasedate").toString());
         * } catch (final Exception e)
         * {
         * logger.severe("FoodTruckStoreGithub.getListAvailableApps Parse Date[" + eElement.getAttribute("releasedate") + " error " + e);
         * }
         * appsItem.description = eElement.getElementsByTagName("description").item(0).getTextContent();
         * // logo -------------- generated by GenerateListingItem.java
         * final String logoSt = eElement.getElementsByTagName("logo").item(0).getTextContent();
         * if (logoSt != null && logoSt.length() > 0)
         * {
         * final Base64 base64 = new Base64();
         * appsItem.logo = base64.decode(logoSt);
         * }
         ...
         * } catch(final Exception e)
         * {
         * storeResult.addEvent(new BEvent(BadListingFormat, e, ""));
         * }
         */
        storeResult.endOperation();
        return storeResult;
    ...
    private boolean match(final TypeApps typeApps, String repositoryName)
    {
        ...
        if (repositoryName.startsWith(typeAppsName + "_")
                || ("CUSTOM" + repositoryName).startsWith(typeAppsName + "_")) {
            return true;
        }
        return false;
    }
    ...
    public FoodTruckResult downloadOneCustomPage(final AppsItem appsItem, final LogBox logBox)
    {
        ...
        if (logBox.isLog(LOGLEVEL.INFO)) {
            logBox.log(LOGLEVEL.INFO, "Download[" + appsItem.getLastUrlDownload() + "]");
        }
